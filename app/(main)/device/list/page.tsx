...
// import MQTTAPI from 'app/api/mqtt/route'
import './style.css'
const TableDemo = () => {
  ...
  const statuses = ['Working', 'Active', 'Inactive']
  ...
  const getData = () => {
    axios.get('http://localhost:3000/api/devices').then(res => {
      setCustomers1(res.data.body)
    })
  }
  ...
  const statusBodyTemplate = (rowData: any) => {
    const statusClassName = `status-${rowData.status.toLowerCase()}`
    return <span className={`customer-badge ${statusClassName}`}>{rowData.status}</span>
  }
      ...
      <div className='col-12'>
        <div className='card'>
          ...
          >
            ...
            <Column field='type' header='Type' filterField='type' style={{ minWidth: '10rem' }} filter filterPlaceholder='Search by type' />
            <Column field='status' header='Status' filterMenuStyle={{ width: '12rem' }} style={{ minWidth: '8rem' }} filter filterElement={statusFilterTemplate} body={statusBodyTemplate} />
            <Column
              field='create_at'
              header='Create_at'
              filterField='create_at'
              style={{ minWidth: '10rem' }}
              filter
              filterPlaceholder='Search by date creation'
              body={rowData => {
                const formattedDate = new Date(rowData.create_at).toLocaleString()
            ...
            <Column
              field='update_at'
              header='Update_at'
              filterField='update_at'
              style={{ minWidth: '10rem' }}
              filter
              filterPlaceholder='Search by time update'
              body={rowData => {
                const formattedDate = new Date(rowData.create_at).toLocaleString()
                return <span>{formattedDate}</span>
              }}
            ...
            <Column header='Description' field='description' filterField='description' style={{ minWidth: '10rem' }} />

            <Column field='activity' header='Activity' showFilterMatchModes={false} style={{ minWidth: '12rem' }} filter filterElement={activityFilterTemplate} />
            <Column field='verified' header='Verified' dataType='boolean' bodyClassName='text-center' style={{ minWidth: '8rem' }} filter filterElement={verifiedFilterTemplate} />
          </DataTable>
        </div>
      </div>

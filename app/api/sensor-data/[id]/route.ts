...
export async function GET(req: NextRequest, { params }: any) {
  ...
  const { count, data, error } = await supabase.from('sensor_reading').select('*', { count: 'exact' }).eq('id', id)
  if (error) {
    return NextResponse.json(
      {
...
export async function DELETE(req: NextRequest, { params }: any) {
  try {
    ...
    const { error } = await supabase.from('sensor_reading').delete().eq('id', idToDelete) //
...
export async function PATCH(req: NextRequest, { params }: any) {
  try {
    ...
    const { error } = await supabase.from('sensor_reading').update(dataToUpdate).eq('id', id)
